...
class AdminWindow(BoxLayout):
    ...
    def add_product_fields(self):
        ...
        crud_price = TextInput(hint_text='Price(Naira)',multiline=False)
        crud_stock = TextInput(hint_text='In Stock(qty)',multiline=False)
        crud_sold = TextInput(hint_text='Sold(qty)',multiline=False)
        crud_order = TextInput(hint_text='Order(Date)',multiline=False)
        crud_purchase = TextInput(hint_text='Last Purchase(Date)',multiline=False)
        crud_submit = Button(text='Add',size_hint_x=None,width=100,on_release=lambda x: self.add_product(crud_code.text,crud_name.text,crud_weight.text,crud_price.text,crud_stock.text,crud_sold.text,crud_order.text,crud_purchase.text))
    ...
    def add_product(self,code,name,weight,price,stock,sold,order,purchase):
        ...
        if code == '' or  name == '' or weight == '' or price =='' or stock == '' or order == '':
            ...
            self.products.insert_one({'product_code':code,'product_name':name,'product_weight':weight,'product_price':price,'in_stock':stock,'sold':sold,'order':order,'last_purchase':purchase})
    ...
    def update_product_fields(self):
        ...
        crud_order = TextInput(hint_text='Order(Date)',multiline=False)
        crud_purchase = TextInput(hint_text='Last Purchase(Date)',multiline=False)
        crud_submit = Button(text='Update',size_hint_x=None,width=100,on_release=lambda x: self.update_product(crud_code.text,crud_name.text,crud_weight.text,crud_price.text,crud_stock.text,crud_sold.text,crud_order.text,crud_purchase.text))
    ...
    def update_product(self,code,name,weight,price,stock,sold,order,purchase):
        ...
        if code == '':
            ...
            self.products.update_one({'product_code':code},{'$set':{'product_code':code,'product_name':name,'product_weight':weight,'product_price':price,'in_stock':stock,'sold':sold,'order':order,'last_purchase':purchase}})
